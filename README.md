...
Иконка рисуется из `build/icon.svg` скриптом `npm run gen:icons`. Тёмный градиент, ночной полумесяц, фиолетовая монета с «K» и зелёный график роста — для тех, кто копит, а не спускает.
